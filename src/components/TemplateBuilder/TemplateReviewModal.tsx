...
export default function TemplateReviewModal({ isOpen, onClose, template }: TemplateReviewModalProps) {
  ...
  const handleUpdateReview = async (reviewId: string, updates: Partial<TemplateReview>) => {
    try {
      const { error } = await updateTemplateReview(reviewId, updates);
      if (error) throw error;
                    ...
                    {editingReview === review.id ? (
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => handleUpdateReview(review.id, { comment: review.comment })}
